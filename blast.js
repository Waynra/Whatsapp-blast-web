...
class BlastManager {
  ...
  getUserSelections() {
    ...
    const delayInput = readlineSync.question(
      chalk.yellow(`Delay between messages (milliseconds) [default: ${config.whatsapp.defaultDelay}]: `)
    );
    ...
    if (sleepIndex === 1) {
      const sleepAfterInput = readlineSync.question(
        chalk.yellow(`Sleep after how many messages? [default: ${config.whatsapp.defaultSleepAfter}]: `)
      );
      sleepAfter = parseInt(sleepAfterInput);
      if (isNaN(sleepAfter) || sleepAfter <= 0) {
        sleepAfter = config.whatsapp.defaultSleepAfter;
        logger.info(`Using default sleep after: ${sleepAfter} messages`);
      }

      const sleepDurationInput = readlineSync.question(
        chalk.yellow(`Sleep duration (milliseconds) [default: ${config.whatsapp.defaultSleepDuration}]: `)
      );
      sleepDuration = parseInt(sleepDurationInput);
      if (isNaN(sleepDuration) || sleepDuration <= 0) {
        sleepDuration = config.whatsapp.defaultSleepDuration;
        logger.info(`Using default sleep duration: ${sleepDuration}ms`);
      }
    }
