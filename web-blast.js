...
class WebBlastManager {
  ...
  async startCampaign(campaignId, data, options) {
    ...
    try {
      ...
      // Cleanup media files from disk after campaign ends/cancels/fails
      if (mediaPath) {
        try {
          let paths = [];
          if (typeof mediaPath === 'string' && mediaPath.trim().startsWith('[')) {
            paths = JSON.parse(mediaPath);
          } else if (Array.isArray(mediaPath)) {
            paths = mediaPath;
          } else {
            paths = [mediaPath];
          }
          ...
          for (const fp of paths) {
            ...
            if (resolvedPath.startsWith(mediaDir) && fs.existsSync(resolvedPath)) {
              fs.unlinkSync(resolvedPath);
              logger.info(`Deleted campaign media file: ${resolvedPath}`);
            }
          }
        } catch (err) {
          logger.error('Error cleaning up campaign media files:', err);
        }
      }
      this.isRunning = false;
    }
